...
def findAllContours(image, thresh):
    ...
    contours = findContours(threshed)  #tag
...
def displayMouth(image, **kwargs):
    ...
    c_1, th, l_c, l_th, r_c, r_th, size_sb = frameData(img, general_thresh)
    c_roi = findMidpoint(kwargs['mouth_roi'][0],kwargs['mouth_roi'][1])
    mid_eyes = findMidpoint(l_c,r_c)

    cali_vector = kwargs['cali_vector']
    cali_c_roi = (mid_eyes[0]+cali_vector[0],mid_eyes[1]+cali_vector[1])

    cali_vector_final = vector(c_roi,cali_c_roi)
    p1 = (int(kwargs['mouth_roi'][0][0]+cali_vector_final[0]),int(kwargs['mouth_roi'][0][1]+cali_vector_final[1]))
    p2 = (int(kwargs['mouth_roi'][1][0]+cali_vector_final[0]),int(kwargs['mouth_roi'][1][1]+cali_vector_final[1]))
    ...
    outline = drawContours(mouth, internals[0], c=255, thresh = mouth_thresh)  #tag
    img[p1[1]:p2[1] + 1,p1[0]:p2[0] + 1] = outline
    outline = cv2.resize(outline, None, fx=1, fy=1, interpolation=cv2.INTER_CUBIC)
...
def setmouthThreshold(video, initial, roi, mouth_roi, cali_vector, thresh):
    ...
    displayKwargs = dict(video=video, winname=winname, thresh_name=thresh_name, start_val=initial,
                         roi=roi, general_thresh=thresh, mouth_roi=mouth_roi, cali_vector=cali_vector)
